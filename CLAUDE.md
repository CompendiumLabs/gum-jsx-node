...
ANSI styling and stdin. A pure library with `@gum-jsx/core` as a peer dependency (`^1.9.0`,
...
- `src/render.ts` - `rasterizeSvg` (PNG `Buffer`) and `rasterizePixels` (`ImageData`) with node-canvas, taking `env` (default: the default Env, via `resolveEnv` from `@gum-jsx/core/env`). Fonts: `registerCanvasFonts(env)` hands the Env's font registry (`env.fonts.paths`) to node-canvas at each draw, tracking what it has registered, so faces registered after this module is imported (the KaTeX faces from `@gum-jsx/math`, a host's own via `env.registerFont`) are still found. Bold and italic faces that are not their own family (`env.fonts.faces`) are registered by their file's registry name; fontconfig matches them by the base family plus weight/style that `Span` emits
...
There is no test suite of its own: the `gum-jsx` suite (`test/run.ts` in `../gum-jsx`) and the
